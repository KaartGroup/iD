...
/*export function svgPropFeatures(projection, context, dispatch) {
    ...
    function showLayer() {
        var layer = context.surface().selectAll('.data-layer.prop');
        layer.interrupt();
...
export function svgPropFeatures(projection, context, dispatch) {
    ...
    function getService() {
        if (services.propFeatures && !_propService) {
            _propService = services.propFeatures;
            //_propService.event.on('loadedData', throttledRedraw);
        }
